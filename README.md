...
- ウィジェットページは同じActivity内で直前の`AppWidgetHostView`を再利用し、同じ寸法のBinder更新も省きます。再利用参照は弱参照なので、非表示ウィジェットを追加でRAMへ固定しません。
...
| `data/LayoutPageOperations.kt` | 右端ページ生成、全空追加ページの圧縮、ページ切替D&D、ウィジェット順序の純粋関数 |
...
26件のユニットテストでは、ホーム／Dock共通フォルダの作成・追加・並べ替え・領域間移動・空フォルダ削除に加え、旧4枠Dockから中央空き5枠への移行、中央枠へのアプリ／フォルダ移動と右端5番目スロット、ページをまたぐ原子的移動、右端ドロップ時のページ生成、先頭ページ保持、末尾／中間空ページの回収、後続ページのセル位置維持、フォルダを持つページの保持、最後の項目移動後のページ削除、ウィジェット順序変更、Appライブラリの表示範囲限定先読みとLow-RAM端末向け予算を検証しています。
