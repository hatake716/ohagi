...
- [ ] 「データを収集しない」「データを共有しない」を申告
  - ohagi はネットワーク通信を行わず、レイアウトデータ(`layout.json`)と、ohagi自身が起動したアプリのコンポーネント・回数・最終時刻(`usage.json`)は端末内の DataStore にのみ保存されます。Androidの利用状況へのアクセス権限は使用しません。
...
- [ ] `POST_NOTIFICATIONS` は、通常起動した1つ目に追加する2つ目を選ぶ常駐通知だけに使用します。Android 13以降では最初のアプリ起動操作の文脈でランタイム許可を一度だけ求めます。拒否時やチャンネル無効時も通常起動は維持し、通知による分割導線だけを表示しません。通知リスナー権限は使用しません。

## 5. ホームランチャー特有の注意

- **デフォルトホーム切替の UX**: ohagi はインストールしただけではホームにならず、ユーザーが自分でデフォルトホームに設定する必要があります。Appライブラリ右上の設定ボタンから RoleManager(Android 10 以降)またはホーム設定画面を開けます。ストア説明文にも切替手順を書いておくと低評価レビューを減らせます。
...
- [ ] ホームまたはAppライブラリから1つ目を通常起動 → 通知の `ohagi` → カテゴリー式ドロワーで2つ目を選択、の順で分割画面になることを確認
- [ ] 通知の `ohagi` をタップした後と分割完了後も通知が残り、同じボタンからピッカーを再度開けることを確認
- [ ] 通知許可／拒否／チャンネル無効の各状態を確認し、拒否／無効時も1つ目の通常起動が妨げられないことを確認
